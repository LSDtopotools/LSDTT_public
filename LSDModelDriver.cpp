...
void LSDModelDriver::ingest_data(string pname, string p_fname)
{
  ...
  while (infile.good())
  {
    ...
    {
      LSDCatchmentModel::CM_support_file_names["hydroindex_file"] = atof(value.c_str());
    }
    else if (lower == "rainfall_data_file")
    {
      support_file_names["rainfall_data_file"] = atof(value.c_str());
    }
    else if (lower == "grain_data_file")
    {
      support_file_names["grain_data_file"] = atof(value.c_str());
    }
    else if (lower == "bedrock_data_file")
    {
      support_file_names["bedrock_data_file"] = atof(value.c_str());
    }
    ...
    {
      LSDCatchmentModel::float_parameters["no_of_iterations"] = atof(value.c_str());
    }
    else if (lower == "min_time_step")
    {
      float_parameters["min_time_step"] = atof(value.c_str());
    }
    else if (lower == "max_time_step")
    {
      float_parameters["max_time_step"] = atof(value.c_str());
    }
    else if (lower == "run_time_start")
    {
      float_parameters["run_time_start"] = atof(value.c_str());
    }
    else if (lower == "max_run_duration")
    {
      float_parameters["max_run_duration"] = atof(value.c_str());
    }
    else if (lower == "memory_limit")
    {
      float_parameters["memory_limit"] = atof(value.c_str());
    }
    else if (lower == "max_time_step")
    {
      float_parameters["max_time_step"] = atof(value.c_str());
    ...
    {
      int_parameters["save_interval"] = atof(value.c_str());
    }
    else if (lower == "time_series_interval")
    {
      int_parameters["time_series_interval"] = atof(value.c_str());
    }
    else if (lower == "elevation_file")
    {
      //bool temp_bool = (value == "true") ? true : false;
      raster_switches["write_elevation_file"] = atof(value.c_str()); //temp_bool;
    }
    else if (lower == "grainsize_file")
    {
      raster_switches["write_grainsize_file"] = atof(value.c_str());
    }
    else if (lower == "parameters_file")
    {
      raster_switches["write_parameters_file"] = atof(value.c_str());
    }    
    else if (lower == "flowvelocity_file")
    {
      raster_switches["write_flowvelocity_file"] = atof(value.c_str());
    }   
    else if (lower == "waterdepth_file")
    {
      raster_switches["write_waterdepth_file"] = atof(value.c_str());
    }   
    else if (lower == "timeseries_file")
    {
      raster_switches["write_timeseries_file"] = atof(value.c_str());
    }
    ...
    {
      method_map["transport_law"] = atof(value.c_str());
    }
    else if (lower == "max_tau_velocity")
    {
      float_parameters["max_tau_velocity"] = atof(value.c_str());
    } 
    else if (lower == "active_layer_thickness")
    {
      float_parameters["active_layer_thickness"] = atof(value.c_str());
    }     
    else if (lower == "recirculate_proportion")
    {
      float_parameters["recirculate_proportion"] = atof(value.c_str());
    }     
    else if (lower == "lateral_erosion_on")
    {
      analyses_switches["lateral_erosion_on"] = atof(value.c_str());
    }     
    else if (lower == "lateral_ero_rate")
    {
      float_parameters["lateral_ero_rate"] = atof(value.c_str());
    }     
    else if (lower == "edge_filter_passes")
    {
      float_parameters["edge_filter_passes"] = atof(value.c_str());
    }     
    else if (lower == "cells_shift_lat")
    {
      float_parameters["cells_shift_lat"] = atof(value.c_str());
    }     
    else if (lower == "max_diff_cross_chann")
    {
      float_parameters["max_diff_cross_chan"] = atof(value.c_str());
    }    
    ...
    {
      analyses_switches["suspended_sediment_on"] = atof(value.c_str());
    }     
    else if (lower == "fall_velocity")
    {
      float_parameters["fall_velocity"] = atof(value.c_str());
    }    
    else if (lower == "grain_size_frac_file")
    {
      support_file_names["grain_size_frac_file"] = atof(value.c_str());
    }      
    ...
    {
      float_parameters["TOPMODEL_m_value"] = atof(value.c_str());
    } 
    else if (lower == "rain_data_time_step")
    {
      int_parameters["rain_data_time_step"] = atof(value.c_str());
    } 
    else if (lower == "spatial_var_rain")
    {
      analyses_switches["spatial_var_rain"] = atof(value.c_str());
    } 
    else if (lower == "input_output_diff")
    {
      float_parameters["input_output_diff"] = atof(value.c_str());
    }     
    else if (lower == "min_Q_for_depth_calc")
    {
      float_parameters["min_Q_for_depth_calc"] = atof(value.c_str());
    }     
    else if (lower == "max_Q_for_depth_calc")
    {
      float_parameters["max_Q_for_depth_calc"] = atof(value.c_str());
    }     
    else if (lower == "depth_ero_threshold")
    {
      float_parameters["depth_ero_threshold"] = atof(value.c_str());
    } 
    else if (lower == "slope_on_edge_cell")
    {
      float_parameters["slope_on_edge_cell"] = atof(value.c_str());
    } 
    else if (lower == "evaporation_rate")
    {
      float_parameters["evaporation_rate"] = atof(value.c_str());
    } 
    else if (lower == "courant_number")
    {
      float_parameters["courant_number"] = atof(value.c_str());
    } 
    else if (lower == "hflow_threshold")
    {
      float_parameters["hflow_threshold"] = atof(value.c_str());
    } 
    else if (lower == "froude_num_limit")
    {
      float_parameters["froude_num_limit"] = atof(value.c_str());
    } 
    else if (lower == "mannings_n")
    {
      int_parameters["mannings_n"] = atof(value.c_str());
    } 
    ...
    {
      analyses_switches["vegetation_on"] = atof(value.c_str());
    }    
    else if (lower == "grass_grow_rate")
    {
      float_parameters["grass_grow_rate"] = atof(value.c_str());
    }   
    else if (lower == "vegetation_crit_shear")
    {
      float_parameters["vegetation_crit_shear"] = atof(value.c_str());
    }   
    else if (lower == "veg_erosion_prop")
    {
      float_parameters["veg_erosion_prop"] = atof(value.c_str());
    } 
    ...
    {
      float_parameters["creep_rate"] = atof(value.c_str());
    } 
    else if (lower == "slope_failure_thresh")
    {
      float_parameters["slope_failure_thresh"] = atof(value.c_str());
    } 
    else if (lower == "soil_erosion_rate")
    {
      float_parameters["soil_erosion_rate"] = atof(value.c_str());
    } 
    else if (lower == "soil_j_mean_depends_on")
    {
      analyses_switches["soil_j_mean_depends_on"] = atof(value.c_str());
    } 
    else if (lower == "call_muddpile_model")
    {
      analyses_switches["call_muddpile_model"] = atof(value.c_str());
    } 
    //=-=-=-=-=-=--=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
