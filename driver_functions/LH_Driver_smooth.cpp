...
int main (int nNumberofArgs,char *argv[])
{
  ...
  LSDRaster Smoothed = DEM.NonLocalMeansFilter(4,2,2,1.25);		

  //Fill 
  float MinSlope = 0.0001;
  LSDRaster FilledDEM = Smoothed.fill(MinSlope);
  ...
  string extra_name = "_smooth_dinf";
  ...
  int thinning = 0;
  string trace_path = "/home/s0675405/DataStore/Final_Paper_Data/OR/clipped/traces/";
  bool basin_filter_switch = false;
  vector<int> Target_Basin_Vector;

  //run HFR    
  vector< Array2D<float> > HFR_Arrays = FlowInfo.HilltopFlowRouting(FilledDEM, hilltops, Surfaces[1], StreamNetwork, aspect, prefix, Basin_Raster, print_paths_switch, thinning, trace_path, basin_filter_switch, Target_Basin_Vector);
  ...
  ofstream WriteData;                 
  stringstream ss;
  ss << path << filename << "_smooth_dinf_PaperData.txt";                
