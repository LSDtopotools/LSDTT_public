...
//
// A usage example is:
// ./LH_Resolution.out /home/s0675405/DataStore/LH_tests/ Oregon 2 1.1 5 0
// 
//
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
//
...
int main (int nNumberofArgs,char *argv[])
{
  ...
  float CriticalSlope = atof(argv[4]); //1.1
  ...
  LSDRaster DEM((path+filename+"_DEM"), "flt");  
 	...
 	//add the output resolution to the prefix to make <prefix>_<resolution>_DEM.flt
  ...
  float window_radius = 6 * resample_res;
  ...
  int threshold = int(1000/resample_res);  //uses ~ same area for each resolution to make this as consistent as possible 
