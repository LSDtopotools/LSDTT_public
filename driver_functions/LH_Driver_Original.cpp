...
int main (int nNumberofArgs,char *argv[])
{
  ...
  //Test for correct input arguments
	if (nNumberofArgs!=6)
	{
		cout << "FATAL ERROR: wrong number inputs. The program needs the path (with trailing slash), the filename prefix, window radius, basin order, critical slope." << endl;
		exit(EXIT_SUCCESS);
	}
  ...
  float CriticalSlope = atof(argv[5]); //1.2 // too high!
...
//	
	...
	//if surface fitting already run
	LSDRaster aspect((path+filename+"_Aspect"),"flt"); 
 	LSDRaster slope((path+filename+"_Slope"),"flt");
 	LSDRaster curvature((path+filename+"_Curvature"),"flt");
  ...
  LSDRaster cht_raster = FilledDEM.get_hilltop_curvature(curvature, Hilltops);
...
//  vector<int> Target_Basin_Vector;
  ...
  //run HFR    
  FlowInfo.HilltopFlowRoutingOriginal(FilledDEM, Hilltops, slope, aspect, StreamNetwork);
...
//    Hilltops.write_raster((path+filename+"_Hilltops"), "flt");
