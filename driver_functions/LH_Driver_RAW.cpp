...
int main (int nNumberofArgs,char *argv[])
{
  ...
  ofstream WriteData;                 
  stringstream ss;
  ss << path << filename << "_PaperData_RAW.txt";                
  ...
  if (WriteRasters == 1){
    ...
    HFR_LH.write_raster((path+filename+"_HFR_LH_RAW"),"flt"); 
    HFR_Slope.write_raster((path+filename+"_HFR_SLP_RAW"),"flt");
    relief.write_raster((path+filename+"_Relief_RAW"),"flt");
