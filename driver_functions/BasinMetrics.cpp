...
//
// Version 1.2.1 - Updated to reflect latest versions of the boomerang plotting stuff.
//
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
//
// Stuart W.D. Grieve
...
int main (int nNumberofArgs,char *argv[])
{
  ...
  //Basins.write_raster("Basins_testing","flt");
  ...
  //slope.write_raster("slope_full","flt");
  //dinf_area.write_raster("area_full","flt");
  ...
  // LH from drainaged density (Tucker et al 2001)
  LSDRaster LH_drainage_density = FilledDEM.HillslopeLengthFromDrainageDensity(StreamNetwork, Basins, FlowDir_array);
  ...
  vector< Array2D<float> > Routed_Hilltop_Data = FilledDEM.HFR_Driver(hilltops, FlowDir_inf, StreamNetwork, Basins, DEM_name);
  ...
  Array2D<float> HillslopeLengths(int(BasinJunctions.size()), 3);
   
  float bin_threshold = 0.05; // move this into param file 
  ...
  for (int q = 0; q < int(Basin_Slopes.size()); ++q){
    ...
    //cout <<"\n" << BasinJunctions[q] << endl;           
    pair<float,float> LH_Pair = Basin_Slopes[q].Boomerang(Basin_Slopes[q], Basin_Drainage_Areas[q], junction_string, log_bin_width, SplineResolution, bin_threshold); 
  ...
  FilledDEM.CollectBasinMetrics(Basins, slope, elevation, aspect, 
                              ...
                              RoutedHilltopSlopes, RoutedHilltopReliefs, RoutedHilltopAspects, LH_drainage_density, HillslopeLengths, critical_slope, DEM_name);
