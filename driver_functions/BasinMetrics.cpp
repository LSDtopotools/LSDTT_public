...
// in future revisoins this will be better dealt with.
//
...
int main (int nNumberofArgs,char *argv[])
{
  ...
  param_file_in >> DEM_name >> DEM_file_extension >> MinSlope >> window_radius >> critical_slope >> log_bin_width;
  param_file_in.close();
  ...
  for (int q = 0; q < int(Basin_Slopes.size()); ++q){
    ...
    Basin_Slopes[q].Boomerang(Basin_Slopes[q], Basin_Drainage_Areas[q], junction_string, log_bin_width);          
  }
