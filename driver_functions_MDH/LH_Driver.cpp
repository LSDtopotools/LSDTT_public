...
int main (int nNumberofArgs,char *argv[])
{
	...
	string ext = "bil";
	//load dem
	LSDRaster DEM((path+filename), ext);	
	...
	//vector<int> sources = FlowInfo.Ingest_Channel_Heads((path+filename+"_dem_CH"), ext); //swap to csv?
	vector<int> sources = FlowInfo.Ingest_Channel_Heads((path+filename+"_CH_wiener_nodeindices_for_Arc"), "csv"); //swap to csv?	
	...
	{
		LSDIndexRaster Floodplains((path+filename+"_FloodPlain"), ext);
	...
	LSDRaster hilltops((path+filename+"_HTs_mask"), ext);	
	...
	cout << "\nWriting preliminary rasters to file" << endl;
		if (WriteRasters == 1){
		cout << "Writing Rasters\n" << endl;																	 
		FilledDEM.write_raster((path+filename+"_Fill"), ext);
		Surfaces[1].write_raster((path+filename+"_Slope"),ext);
		Surfaces[2].write_raster((path+filename+"_Aspect"),ext);
		Surfaces[3].write_raster((path+filename+"_Curvature"),ext);
		StreamNetwork.write_raster((path+filename+"_STNET"), ext); 
		Basin_Raster.write_raster((path+filename+"_Basins"), ext); 
		hilltops.write_raster((path+filename+"_HTs"), ext);
		CHT.write_raster((path+filename+"_CHT"),ext);
	}
	...
	if (WriteRasters == 1){
		cout << "Writing Rasters\n" << endl;																	 
		HFR_LH.write_raster((path+filename+"_HFR_LH"),ext); 
		HFR_Slope.write_raster((path+filename+"_HFR_SLP"),ext);
		relief.write_raster((path+filename+"_Relief"),ext);
	}
}
