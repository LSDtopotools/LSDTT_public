...
int main (int nNumberofArgs,char *argv[])
{
  ...
  if (lchar != slash)
  {
	...
	for (int i=0; i<N; ++i)
	{
		...
		string ChannelFileName = "_ChanNet";
		string Ext = ".chan";
		string Chan_for_chi_ingestion_fname = path_name+DEM_name+ChannelFileName+JunctionName+Ext;
		...
		string mn_fo_fname = (path_name+DEM_name+bfchi_fname+param_str+JunctionName+fpt_ext);
