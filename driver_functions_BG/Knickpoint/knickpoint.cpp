//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
//
// m_chi knickpoint mapping tool
//
// This program takes two arguments, the path name and the driver name
// The driver file has a number of options that allow the user to calculate
// different kinds of chi analysis, based on the Chi mapping tool
// from Mudd et al., 2014, modified to detect knickpoints based on
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
//
...
int main (int nNumberofArgs,char *argv[])
{
  ...
  float_default_map["threshold_knickpoint_length"] = 150000; // length threshold to avoid too close knickpoints
