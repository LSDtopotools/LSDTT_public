	...
	for (i=1; i<NRows-1; ++i)
  {
    ...
    for (j=1; j<NCols-1; ++j) 
    {
			...
			if (hilltops[i][j] != NoDataValue) {
				...
				//continue trace until a stream node is encountered
				while (flag == true && a > 0 && a < NRows-1 && b > 0 && b < NRows-1) {   //added boudary checking to catch cells which flow off the	edge of the DEM tile.
				  int a_2 = a;
          int b_2 = b;
...
// others, and the code will run as normal.
//
// New version added to exclude traces which pass over positive planform curvature
// ie divergent slopes. This allows the results to be directly compared to 1D models
// SWDG - 18/11/14
//
// The structure of the returned vector< Array2D<float> > is as follows:
// [0] Hilltop Network coded with stream ID
...
// SWDG 12/2/14
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
vector< Array2D<float> > LSDFlowInfo::HilltopFlowRouting_Divergence(LSDRaster Elevation, LSDRaster Hilltops, LSDRaster Slope,
    ...
    for (j=1; j<NCols-1; ++j) {
          ...
          if(zeta[a][b] - zeta[a_2][b_2] > 0){
            ...
            b_2 = b;
				...
				if (a == 0 || b == 0 ||	a == NRows-1 || b == NCols-1 || stnet[a][b] != NoDataValue || stnet[a-1][b-1] != NoDataValue || stnet[a][b-1] != NoDataValue || stnet[a+1][b-1] != NoDataValue || stnet[a+1][b] != NoDataValue || stnet[a+1][b+1] != NoDataValue || stnet[a][b+1] != NoDataValue || stnet[a-1][b+1] != NoDataValue || stnet[a-1][b] != NoDataValue || path[a][b] >= 3 || PlanCurvature.get_data_element(a,b) < (-0.00001) || PlanCurvature.get_data_element(a,b) > (0.00001)) flag = false;
				}
				  ...
				  if (stnet[a][b] != NoDataValue || stnet[a-1][b-1] != NoDataValue || stnet[a][b-1] != NoDataValue || stnet[a+1][b-1] != NoDataValue || stnet[a+1][b] != NoDataValue || stnet[a+1][b+1] != NoDataValue || stnet[a][b+1] != NoDataValue || stnet[a-1][b+1] != NoDataValue || stnet[a-1][b] != NoDataValue)
          {
            ...
            if (relief > 0){
              ...
              ++neg_count;
				  ...
				  else if (PlanCurvature.get_data_element(a,b) < (-0.00001)){
            ++divergent_count;
          }
				  else{  //unable to route using aspects
				    ofs << "fail: " << a << " " << b << " " << i << " " << j << endl;
  ...
  cout << "Uphill count: " << neg_count << endl;
  cout << "Edge count: " << edge_count << endl;
  cout << "Divergent count: " << divergent_count << endl;
