...
int main (int nNumberofArgs,char *argv[])
{
  ...
  string complete_fname = DEM_name+sources_ext;//".asc";

  // load the DEM
  LSDRaster topo_test(DEM_name, DEM_asc_extension);
  ...
  filled_topo_test.write_raster((DEM_f_name),DEM_asc_extension);
  ...
  //LSDIndexRaster NodeIndex = FlowInfo.write_NodeIndex_to_LSDIndexRaster();
	//NodeIndex.write_raster((path_name+DEM_name+NI_name), DEM_asc_extension);
  ...
  for(int i = 0; i<int(raster_selection.size()); ++i)
  {
    if(raster_selection[i]==1)
    {
      ...
      tan_curvature.write_raster((path_name+DEM_name+curv_name), DEM_asc_extension);
    }
  }
  ...
  //LSDIndexRaster ValleyJunctionsRaster (NRows,NCols,XMinimum,YMinimum,DataResolution,NoDataValue,valley_junctions);
	//ValleyJunctionsRaster.write_raster((path_name+DEM_name+VJ_name),DEM_asc_extension);
...
//   string prefix = "/home/smudd/LSDTopoData/LiDAR_datasets/USA/OH/indian_creek2_fill_nodeindices_for_Arc";
  ...
  Channel_heads_raster.write_raster((DEM_name+CH_name),DEM_asc_extension);
  ...
  SOArrayNew.write_raster((DEM_name+SO_name_new),DEM_asc_extension);
