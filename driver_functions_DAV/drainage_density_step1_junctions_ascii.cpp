...
int main (int nNumberofArgs,char *argv[])
{
  ...
  vector<string> boundary_conditions(4);
	...
	boundary_conditions[3] = "No flux";
		...
		// get the filled file
	LSDRaster filled_topo_test((path_name+DEM_name+fill_ext), DEM_asc_extension);
  
  //get a FlowInfo object
	LSDFlowInfo FlowInfo(boundary_conditions,filled_topo_test); 
  
  //get the sources from raster to vector
  vector<int> sources = FlowInfo.Ingest_Channel_Heads((path_name+sources_name),DEM_asc_extension);
  
	// now get the junction network
	LSDJunctionNetwork ChanNetwork(sources, FlowInfo);
	LSDIndexRaster SOArray = ChanNetwork.StreamOrderArray_to_LSDIndexRaster();
	string SO_name = "_SO";
	SOArray.write_raster((path_name+DEM_name+SO_name), DEM_asc_extension);
  ...
  //Curvature.write_raster((path_name+DEM_name+curv_name), DEM_asc_extension);
  ...
  CHT.write_raster((path_name+DEM_name+CHT_name), DEM_asc_extension);
