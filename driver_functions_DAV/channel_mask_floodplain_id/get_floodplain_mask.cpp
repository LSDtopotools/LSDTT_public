//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
//
// get_floodplain_channel_mask
//
// Attempts to get a mask for the floodplain and the channel areas
//
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
//
// 19/02/2017
...
//
// Based on:
// Fiona J. Clubb's driver and algorithms
...
int main (int nNumberofArgs,char *argv[])
{
  ...
  int threshold_SO, FilterTopo, window_radius,
      ...
      minimum_patch_size, junction_number, search_distance;
  ...
  // calcualte the distance from outlet
	...
	// load the sources
  vector<int> sources = FlowInfo.Ingest_Channel_Heads((path_name+DEM_ID+CH_name),
                                                      csv_extension, 2);
  ...
  LSDRaster ChannelRelief =
      ChanNetwork.calculate_relief_from_channel(filled_topo_test,
                                                FlowInfo,
                                                threshold_SO);
  ...
  surface_fitting =
      filled_topo_test.calculate_polyfit_surface_metrics(surface_fitting_window_radius,
                                                         raster_selection);
  ...
  // DAV can be commented out for channel area ID - must be user set
  ...
  float relief_threshold_from_qq =
      ChannelRelief.get_threshold_for_floodplain_QQ(qq_fname,
                                                    threshold_condition,
                                                    lower_percentile,
                                                    upper_percentile);
  ...
  float slope_threshold_from_qq =
      Slope.get_threshold_for_floodplain_QQ(qq_slope,
                                            threshold_condition,
                                            lower_percentile,
                                            upper_percentile);
	
  cout << "Relief threshold: " << relief_threshold_from_qq <<
          " Slope threshold: " << slope_threshold_from_qq << endl;
  ...
  LSDFloodplain Floodplain(ChannelRelief,
                           Slope,
                           ChanNetwork,
                           FlowInfo,
                           relief_threshold_from_qq,
                           slope_threshold_from_qq,
                           minimum_patch_size,
                           threshold_SO);
	
	// get the relief relative to nearest channel
  Floodplain.Get_Relief_of_Nearest_Channel(ChanNetwork,
                                           FlowInfo,
                                           filled_topo_test,
                                           DistFromOutlet,
                                           threshold_SO,
  ...
  relief_final.write_raster((path_name+DEM_ID+relief_ext),
                            DEM_extension);
	
  // Write the floodplain raster
