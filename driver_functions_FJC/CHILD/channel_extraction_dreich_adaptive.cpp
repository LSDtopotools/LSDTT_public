//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
// channel_extraction_dreich_adaptive.cpp
...
// It uses an adaptive curvature threshold to identify valleys on which to run the
// algorithm.
//
...
int main (int nNumberofArgs,char *argv[])
{
  ...
  LSDIndexRaster connected_components = raster.IsolateChannelsWienerQQAdaptive(area_threshold, window_radius, Output_name+".txt");
