...
int main (int nNumberofArgs,char *argv[])
{
  ...
  //float XMinimum = filled_topo_test.get_XMinimum();
  //float YMinimum = filled_topo_test.get_YMinimum();
  //float DataResolution = filled_topo_test.get_DataResolution();
  ...
  int half_width = 100;
  Array2D<float> tan_curv_threshold = tan_curvature.CalculateAdaptiveCurvatureThresholdSD(half_width);
  Array2D<float> tan_curv_array = tan_curvature.get_RasterData();
  
  //float tan_curv_threshold = 0.01;
	Array2D<int> valley_junctions = ChanNetwork.find_valleys_adaptive_threshold(FlowInfo, tan_curv_array, sources, no_connecting_nodes, tan_curv_threshold);
