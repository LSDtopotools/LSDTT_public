...
// Extract information about terraces using an LSDIndexChannel object. User must specify
// the starting and ending junction of the channel.  IMPORTANT -
// the starting junction is the UPSTREAM one.
// Could pass in lat long coordinates instead? can add this later.
//
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
//
// Fiona Clubb
...
int main (int nNumberofArgs,char *argv[])
{
  ...
  cout << "The path name is: " << path_name << " and the filename is: " << f_name << endl;
	...
	string DEM_ID, CH_name,temp;
	int StartingJN, EndingJN, threshold_SO, FilterTopo, lower_percentile_relief, upper_percentile_relief, lower_percentile_slope, upper_percentile_slope, minimum_patch_size, search_distance;
